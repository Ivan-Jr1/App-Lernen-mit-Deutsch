...
def test_dashboard_agrega_pontos_streak_e_totais(client, auth):
    ...
    esposa_row = next(u for u in dashboard["users"] if u["username"] == "esposa")
    ...
    assert esposa_row["total_points"] == 0
    assert esposa_row["current_streak"] == 0
    assert esposa_row["cards_due_today"] == 2
...
def test_dashboard_nao_lista_o_visitante(client, guest_headers):
    ...
    assert {u["username"] for u in dashboard["users"]} == {"ivan", "esposa"}
